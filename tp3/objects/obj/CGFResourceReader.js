 ...
 */
export class CGFresourceReader {
  constructor() {
    this.xmlhttp = null;
    this.xmlDoc = null;
    this.resource = null;
   ...
   */
  open(resource, callbackObj) {
    this.resource = resource;
    if (!callbackObj.onResourceReady)
      console.error(
        "CGFresourceReader.open: onResourceReady handler not defined."
      );

    if (!callbackObj.onResourceError)
      console.error(
        "CGFresourceReader.open: onResourceError handler not defined."
      );
  ...
  onStateChange() {
    if (this.readyState == 4) {
      ...
      if (this.status == 200) {
        ...
        var reader = this.reader;
        reader.callbackObj.onResourceError(
          this.status + ": " + reader.resource + ", " + this.statusText
        );
      }
    }
  }
